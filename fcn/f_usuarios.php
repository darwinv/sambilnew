...
function getUser() {
	...
	$foto = new fotos();
	$valores ['ruta'] = $foto->buscarFotoUsuario($usuarios_id);
...
function newUser() {
	...
	if (isset ( $_POST ["type"] )) {
		...
		if ($tipo == "empresa") {
			$rif = filter_input ( INPUT_POST, "p_identificacion" );
			...
			$usuario->datosJuridico ( $rif, filter_input ( INPUT_POST, "e_razonsocial" ), filter_input ( INPUT_POST, "p_tipo" ), filter_input ( INPUT_POST, "e_categoria" ) );
		} else {
