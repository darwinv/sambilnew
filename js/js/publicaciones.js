...
//						$("#ajaxContainer3").html(data);
						...
						*/
						$("#pub-form-date").formValidation({
							...
							fields:{
								txtFecha:{validators:{
								date:{format:'DD/MM/YYYY',min:$("#txtFecha").attr("min"),max:$("#txtFecha").attr("max")}}}
							}
						}).on('success.form.fv',function(e){
							/*
							...
							form = $("#pub-form-reg").serialize() +"&fecha=" + $("#txtFecha").val()+"&idclas="+$(".form-select-publicar").last().val()+"&monto="+$("#txtPrecio").autoNumeric("get")+"&metodo=guardar"+fotos;
							$.ajax({
								url:'paginas/publicar/fcn/f_publicaciones.php',
								data:form,
								type : "POST",
